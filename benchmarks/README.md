...
There are **three independent version namespaces** in this repository. They are
deliberately separate and must not be conflated:

| Namespace | Source of truth | Current value | Purpose |
|-----------|-----------------|---------------|---------|
| Engine version | `predictron_engine/version.py` → `ENGINE_VERSION` | `0.12.1` | Canonical engine release. **Deliberately stable**: feature/intelligence sprints do not bump it (regression-tested in `tests/engine/test_intelligence_sprint9.py`). Stamped into every `Report.analysis_metadata.engine_version`. |
| Snapshot version | Filename under `benchmarks/expected_outputs/` | latest committed `0.13.0` | **Historical regression baseline**, frozen at snapshot-save time. Used only to detect regressions between committed snapshots. |
| Benchmark framework version | `snapshot["benchmark_version"]` in `benchmark_runner.save_snapshot()` | `2.0.0` | Version of the *benchmark tooling*, constant across snapshots. |

The application version (`app.core.config.APP_VERSION`, e.g. `1.0.0`) is a fourth,
separate namespace for the HTTP API surface and is reported by the health endpoint;
it is not the engine version.

### Why the latest snapshot (`0.13.0`) predates `ENGINE_VERSION` (`0.12.1`)

The `v0.13.0` snapshot was generated by the "(Intelligence) Sprint 13" benchmark
run, **before** the canonical `predictron_engine/version.py` was introduced in the
subsequent architecture-consistency cleanup. Since then the engine version has been
deliberately held at `0.12.1` across feature sprints, while the committed snapshot
kept its original name.

The snapshot is therefore a **historical baseline in the snapshot namespace**, not a
claim that the current engine is version `0.13.0`. It is intentionally **not
re-versioned or re-generated**: snapshot regeneration requires live evidence
collection and is explicitly non-reproducible in CI, so re-stamping the file would
both fabricate a version and produce an unverifiable artifact.

### CI benchmark diff

CI diffs the **two most recent committed snapshots** (currently `0.12.0` → `0.13.0`)
using `benchmark_report --diff`, which exits non-zero on regressions. The workflow
auto-discovers these two snapshots from the committed `expected_outputs/` tree, so a
newly committed snapshot is picked up without editing the workflow. This checks
*snapshot-to-snapshot* regression only; comparing raw engine output against a
snapshot cannot run in CI because live evidence collection is non-deterministic.

Engine releases are versioned and snapshotted *together*: when a release bumps
`ENGINE_VERSION`, a matching `snapshot_v{ENGINE_VERSION}.json` should be committed so
the two namespaces stay aligned.
